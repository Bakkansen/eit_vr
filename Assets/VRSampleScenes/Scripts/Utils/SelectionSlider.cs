...
namespace VRStandardAssets.Utils
{
    ...
    public class SelectionSlider : MonoBehaviour
    {
        ...
        private void Update ()
        {
            if(!m_UIFader || !m_isStartBar || m_isIntroBar)
                return;

            // If this bar is using a UIFader turn off the collider when it's invisible.
            m_Collider.enabled = m_UIFader.Visible || m_isStartBar || m_isIntroBar;
        }
        ...
        public void SetText(string text) {
            Text m_text = m_Slider.GetComponentInChildren<Text>();
            m_text.text = text;
        }
        ...
        public void SetAnswerScale(float s) {
            m_answerScale = s;
        }

        public float GetAnswerScale() {
            return m_answerScale;
        }
    }
}